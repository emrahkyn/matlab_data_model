...
for i=1:length(allParameters(:,1))
    ...
    if deltaT ~= 0
        ...
        %inputArr(index,2:length(indexOfMeasurement)+1)=deltaParameter/deltaT;
        inputArr(index,1:length(indexOfMeasurement))=deltaParameter;
...
corrMatrix = corr(input,'Type','Spearman');
writematrix(corrMatrix,'../output/correlations/ActualValueSpearmanCorrelations.xlsx')